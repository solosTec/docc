...
namespace tex
{
	...
	void to_tex(std::ostream& os, double d) {
	...
	void to_tex(std::ostream& os, cyng::vector_t const& vec, std::string sep) {
		...
		for (auto const& obj : vec) {
			...
			switch (obj.rtti().tag()) {
			case cyng::TC_VECTOR:
				to_tex(os, cyng::container_cast<cyng::vector_t>(obj), " ");
				break;
			case cyng::TC_UINT64:
				//os << "UINT64";
				os
			...
			case cyng::TC_INT64:
				...
				break;
			case cyng::TC_DOUBLE:
				to_tex(os, cyng::numeric_cast<double>(obj, 0.0));
				break;
			case cyng::TC_STRING:
				os << obj;
				break;
	...
	void esc_tex(std::ostream& os, std::string const& s) {
		for (auto c : s) {
			...
			case '‡': os << "\\ddag "; break;	//	double dagger
