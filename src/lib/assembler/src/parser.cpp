...
namespace docasm {
	...
	bool parser::state_type(symbol const& sym) {
		...
		state_.pop();
		//
		//	generate a value of the previous specified type
		//
	...
	bool parser::state_value(symbol const& sym) {
		...
		case symbol_type::TYP:
			//	data type => value
			//	example:
			//	'uuid:bfdf66e3-04f7-4fcd-ba0e-e3e3fee06cd9'
			prg_.init_literal(sym.value_);
			state_.push(nonterminal(sym, nonterminal_type::TYPE));
			break;
		default:
			fmt::print(
				stdout,
	...
	void parser::statement(symbol const& sym) {
		...
		if (pos != ops_.end()) {
			...
			case cyng::op::FORWARD:
				...
				BOOST_ASSERT_MSG(false, "FORWARD not supported yet");
				break;
			case cyng::op::JA:
