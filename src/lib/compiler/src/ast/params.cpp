...
namespace docscript {
	...
	std::size_t param::compile(std::function< void(std::string const&) > emit, std::size_t depth, std::size_t index) const
	{
	    ...
	    return (next_) ? next_->compile(emit, depth, index + 1) + 1u : 1u;
	}
	void param::transform(context const& ctx)
	{
		...
		if (optm) { value_.rename(*optm); }
	    }
	    if (next_) { next_->transform(ctx); }
	}
	...
	bool param::is_complete() const { return !value_.empty(); }
	...
	{
	    if (next_) { return next_->append(std::move(p)); }
	    else {
	...
	std::size_t param::size() const
	{
	    ...
	    while (p != nullptr) {
		++n;
		p = (p->next_) ? p->next_.get() : nullptr;
	    }
	    return n;
	}

	param param::factory(symbol const& sym) { return { sym.value_ }; }

	std::ostream& operator<<(std::ostream& os, param const& p)
	{
	    os << "param[" << p.key_ << ":" << p.value_;
	    if (p.next_) { os << "->" << *(p.next_); }
	    os << "]";
	    return os;
	}
    }
