...
namespace docruntime {
    ...
    int controller::run(
        ...
        if (!ofs_.is_open() || !tmp_html_.is_open()) {
            fmt::print(stdout, fg(fmt::color::dark_orange) | fmt::emphasis::bold, "***info : cannot open output file\n");
            return EXIT_FAILURE;
        }
        ...
        if (!r.second) {
            fmt::print(
                stdout, fg(fmt::color::dark_orange) | fmt::emphasis::bold, "***error: input file [{}] not found\n", inp.string());
            return EXIT_FAILURE;
        }
        if (ctx_.get_verbosity(2)) {
            fmt::print(stdout, fg(fmt::color::gray), "***info : input file [{}]\n", r.first.string());
        }
        ...
        if (ctx_.get_verbosity(2)) {
            ...
            fmt::print(stdout, fg(fmt::color::forest_green), "***info : intermediate file {} complete\n", ctx_.get_output_path());
        }
        ...
        if (ctx_.get_verbosity(2)) {
            ...
            fmt::print(
                stdout, fg(fmt::color::forest_green), "***info : program {} is loaded\n", assembler_.get_output_path().string());
        }
        ...
        if (ifs.is_open()) {
            ...
            if (ctx_.get_verbosity(4)) {
                ...
                fmt::print(stdout, fg(fmt::color::forest_green), "***info : TOC:\n");
            ...
            if (ofs_index.is_open()) {
                ...
                ofs_index << cyng::io::to_json_pretty(cyng::make_object(vec)) << std::endl;
            } else {
                fmt::print(
                    stderr,
            ...
            fmt::print(
                stdout, fg(fmt::color::dark_orange) | fmt::emphasis::bold, "***info : input file [{}] not found\n", inp.string());
            return EXIT_FAILURE;
        }

        auto const delta = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - now);

        fmt::print(stdout, fg(fmt::color::forest_green), "***info : complete after {} milliseconds\n", delta.count());
    ...
    void controller::emit_header(cyng::param_map_t &meta) {
        ...
        for (auto const &param : meta) {
            if (boost::algorithm::equals(param.first, "title")) {
                ...
                ofs_ << "\t<meta name=\"twitter:title\" content=\"" << pos->second << "\" />" << std::endl;
    ...
    void emit_styles(std::size_t depth, std::ostream &ofs) {
        ...
        ofs << std::string(depth, '\t') << "<style>" << std::endl
            ...
            << std::string(depth + 2, '\t') << "--color-code-function: #74531F;"
            << std::endl //	function and function templates
            ...
            << std::string(depth + 2, '\t') << "--color-code-macro: #8A1BFF;"
            << std::endl
            ...
            << std::string(depth + 2, '\t') << "--color-code-parameter: #808080;"
            << std::endl
            ...
            << std::string(depth + 2, '\t') << "font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;"
            << std::endl
            ...
            << std::string(depth + 1, '\t') << "code, kbd, pre, samp {" << std::endl
            << std::string(depth + 1, '\t') << "\tfont-family: monospace;" << std::endl
            << std::string(depth + 1, '\t') << "}"
            << std::endl
            ...
            << std::string(depth + 1, '\t') << "blockquote > p { margin-bottom: 1px; }" << std::endl
            << std::string(depth + 1, '\t') << "pre { background-color: #fafafa; }" << std::endl

            << std::string(depth + 1, '\t') << "pre > code:hover {" << std::endl
            << std::string(depth + 2, '\t') << "background-color: orange;" << std::endl
            << std::string(depth + 1, '\t') << "}" << std::endl

            << std::string(depth + 1, '\t') << "blockquote {" << std::endl
            << std::string(depth + 2, '\t') << "border-left: 4px solid #eee;" << std::endl
            ...
            << std::string(depth + 1, '\t') << "}" << std::endl
            << std::string(depth + 1, '\t') << "figure > figcaption {" << std::endl
            ...
            //<< std::endl
            << std::string(depth + 2, '\t') << "border: 2px solid #777;" << std::endl
            << std::string(depth + 1, '\t') << "}" << std::endl
            << std::string(depth + 1, '\t') << "img:hover {" << std::endl
            << std::string(depth + 2, '\t') << "box-shadow: 0 0 10px #ccc;" << std::endl
            << std::string(depth + 1, '\t') << "}"
            << std::endl
            ...
            << std::string(depth + 2, '\t') << "background-color: #eee;" << std::endl
            << std::string(depth + 1, '\t') << "}" << std::endl

            << std::string(depth + 1, '\t') << "div.smf-svg:hover {" << std::endl
            << std::string(depth + 2, '\t') << "box-shadow: 0 0 10px #ccc;" << std::endl
            << std::string(depth + 1, '\t') << "}"
            << std::endl
            ...
            << std::string(depth + 2, '\t') << "flex-flow: row wrap;" << std::endl
            << std::string(depth + 1, '\t') << "}" << std::endl
            ...
            << std::string(depth + 2, '\t') << "content: \":\";" << std::string(depth + 1, '\t') << "}" << std::endl
            ...
            << std::string(depth + 1, '\t') << "ul.alert > li:nth-child(2) {" << std::endl
            << std::string(depth + 2, '\t') << "background-color: #ddd;" << std::endl
            << std::string(depth + 2, '\t') << "border-radius: 5px;" << std::endl
            << std::string(depth + 1, '\t') << "}"
            << std::endl

            //	formatting sub and supscript
            << std::string(depth + 1, '\t') << "sup { top: -.5em; }" << std::endl
            << std::string(depth + 1, '\t') << "sub { bottom: -.25em; }" << std::endl
            << std::string(depth + 1, '\t') << "sub, sup {" << std::endl
            << std::string(depth + 2, '\t') << "font-size: 75%;" << std::endl
            << std::string(depth + 2, '\t') << "line-height: 0;" << std::endl
            << std::string(depth + 2, '\t') << "position: relative;" << std::endl
            << std::string(depth + 2, '\t') << "vertical-align: baseline;" << std::endl
            << std::string(depth + 1, '\t') << "}"
            << std::endl
            ...
            << std::string(depth + 2, '\t') << "position: absolute;" << std::endl
            << std::string(depth + 2, '\t') << "right: 2em;" << std::endl
            << std::string(depth + 2, '\t') << "width: 20%;" << std::endl
            << std::string(depth + 2, '\t') << "border: 1px #D5DBDB solid;" << std::endl
            ...
            << std::string(depth + 2, '\t') << "border-collapse: collapse;" << std::endl
            << std::string(depth + 2, '\t') << "border-spacing: 0px;" << std::endl
            << std::string(depth + 1, '\t') << "}"
            << std::endl
            ...
            << std::string(depth + 2, '\t') << "background-color: DimGray;" << std::endl
            << std::string(depth + 2, '\t') << "padding: 5px;" << std::endl
            ...
            << std::string(depth + 2, '\t') << "background-color: #333333;" << std::endl
            << std::string(depth + 2, '\t') << "border-radius: 3px;" << std::endl
            << std::string(depth + 1, '\t') << "}"
            << std::endl
            ...
            << std::string(depth + 2, '\t') << "list-style-type: none;" << std::endl
            << std::string(depth + 2, '\t') << "text-decoration: none;" << std::endl
            << std::string(depth + 2, '\t') << "margin-left: 0em;" << std::endl
            << std::string(depth + 2, '\t') << "padding-left: 0.7em;" << std::endl
            << std::string(depth + 1, '\t') << "}"
            << std::endl
            ...
            << std::string(depth + 2, '\t')
            << "font-family: ui-monospace, SFMono-Regular, 'SF Mono', Menlo, Consolas, 'Liberation Mono', monospace;" << std::endl
