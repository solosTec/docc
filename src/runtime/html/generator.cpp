...
namespace docruntime {
    ...
    generator::generator(
        ...
        if (ctx_.get_verbosity(4)) {
            ...
            fmt::print(
                stdout, fg(fmt::color::forest_green), "***info : default language is [{}]\n", loc.at(cyng::sys::info::LANGUAGE));
        }
        ...
        //	get statistics
        //
        for (auto const &r : ctx_.get_stats()) {
            meta_.emplace("stats-" + to_string(r.first), cyng::make_object(std::to_string(r.second)));
        }
    }

    cyng::param_map_t &generator::get_vars() { return vars_; }
    cyng::param_map_t &generator::get_meta() { return meta_; }
    docruntime::toc &generator::get_toc() { return toc_; }
    ...
    void generator::label(cyng::vector_t vec) {
        ...
        os_ << "<a id=\"" << boost::uuids::to_string(id) << "\" style = \"display:inline;\" href=\"" << name << "\"></a>";
    ...
    void generator::emit_header(std::size_t level, boost::uuids::uuid tag, std::string const &num, std::string const &title) {
        ...
        os_ << "<h" << level + 1 << ">" << num << "&nbsp;" << title << "<a id=\"" << tag << "\" aria-hidden=\"true\" "
            << "href=\"#" << tag << "\""
            << "class=\"oction\">"
    ...
    void generator::figure(cyng::param_map_t pm) {
        ...
        if (r.second) {
            ...
            if (scale > 1.0 || scale < 0.01) {
                std::cerr << "***warning: unusual scaling factor [" << scale << "] for figure: " << source << std::endl;
            }
            ...
            os_ << "<p>" << source << " not found</p>" << std::endl;
        }
    }

    void generator::embed_svg(
    ...
    void generator::embed_base64(
        ...
        auto const title = compute_title_figure(tag, caption);
        auto const figure = dom::figure(
            dom::id_(id),
            dom::img(
                dom::alt_(alt),
                dom::title_(caption),
                dom::class_("docscript-img"),
                dom::style_("max-width: " + max_width),
                dom::src_("data:image/" + ext + ";base64," + cyng::crypto::base64_encode(buffer.data(), buffer.size()))),
            dom::figcaption(title));
        figure.serialize(os_);
    }

    void generator::table_of_content(cyng::param_map_t) {
        os_ << "<details>\n"
            << "\t<summary>" << get_name(get_language_code(), i18n::TOC) << "</summary>\n";
    ...
    void generator::emit_toc(cyng::vector_t vec, std::size_t level) {
        ...
        os_ << std::string(level + 1, '\t') << "<ul toclevel-" << level << ">" << std::endl;
        ...
        for (auto const &obj : vec) {
            ...
            if (sub) {
                os_ << std::string(level + 1, '\t') << "<li>" << a(0) << std::endl;

                emit_toc(cyng::container_cast<cyng::vector_t>(sub), level + 1);

                os_ << std::string(level + 1, '\t') << "</li>" << std::endl;

            } else {
                os_ << std::string(level + 1, '\t') << "<li>" << a(0) << "</li>" << std::endl;
            }
        }

        os_ << std::string(level + 1, '\t') << "</ul>" << std::endl;
    }
    ...
    void generator::code(cyng::param_map_t pm) {
        ...
        if (r.second) {
            ...
            fmt::print(fg(fmt::color::dark_orange) | fmt::emphasis::bold, "***error: source file [{}] not found\n", source);
    ...
    void generator::resource(cyng::param_map_t pm) { std::cout << "RESOURCE(" << pm << ")" << std::endl; }
    std::chrono::system_clock::time_point generator::now(cyng::param_map_t pm) { return std::chrono::system_clock::now(); }
    boost::uuids::uuid generator::uuid(cyng::param_map_t) { return uuid_gen_(); }

    cyng::vector_t generator::range(cyng::vector_t vec) { return vec; }
    ...
    std::function<void(cyng::param_map_t)> generator::f_set() { return std::bind(&generator::set, this, std::placeholders::_1); }
    std::function<cyng::vector_t(cyng::vector_t)> generator::f_get() {
        return std::bind(&generator::get, this, std::placeholders::_1);
    }
    std::function<void(cyng::param_map_t)> generator::f_meta() { return std::bind(&generator::meta, this, std::placeholders::_1); }
    ...
    std::function<void(cyng::vector_t)> generator::f_label() { return std::bind(&generator::label, this, std::placeholders::_1); }
    std::function<std::string(cyng::vector_t)> generator::f_ref() {
        return std::bind(&generator::ref, this, std::placeholders::_1);
    }
    std::function<void(cyng::vector_t)> generator::f_h1() { return std::bind(&generator::h1, this, std::placeholders::_1); }
    std::function<void(cyng::vector_t)> generator::f_h2() { return std::bind(&generator::h2, this, std::placeholders::_1); }
    std::function<void(cyng::vector_t)> generator::f_h3() { return std::bind(&generator::h3, this, std::placeholders::_1); }
    std::function<void(cyng::vector_t)> generator::f_h4() { return std::bind(&generator::h4, this, std::placeholders::_1); }
    std::function<void(cyng::vector_t)> generator::f_h5() { return std::bind(&generator::h5, this, std::placeholders::_1); }
    std::function<void(cyng::vector_t)> generator::f_h6() { return std::bind(&generator::h6, this, std::placeholders::_1); }
    ...
    std::function<void(cyng::param_map_t)> generator::f_code() { return std::bind(&generator::code, this, std::placeholders::_1); }

    std::function<void(cyng::param_map_t)> generator::f_tree() { return std::bind(&generator::tree, this, std::placeholders::_1); }
    ...
    std::string generator::compute_title_figure(boost::uuids::uuid tag, std::string caption) {
        ...
        std::stringstream ss;
        ss << get_name(get_language_code(), i18n::FIGURE) << ": " << idx << " - " << caption;
    ...
    std::string generator::compute_title_table(boost::uuids::uuid tag, std::string caption) {
        ...
        std::stringstream ss;
        ss << get_name(get_language_code(), i18n::TABLE) << ": " << idx << " - " << caption;
    ...
    cyng::io::language_codes generator::get_language_code() const {
        ...
        return (lang.size() == 2) ? cyng::io::get_language_code(lang.at(0), lang.at(1)) : cyng::io::LC_EN;
    }
    ...
    std::string generator::get_language() const {
        ...
        return (lang.size() == 2) ? boost::algorithm::to_lower_copy(lang) : "en";
    }
