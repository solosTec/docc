...
int main(int argc, char *argv[]) {
    ...
    generic.add_options()
        ...
        ("help,h", "print usage message")("version,v", "print version string") //   -h
    ...
    gen.add_options()                                                                                            //  all options
        ("generator.root", boost::program_options::value(&root)->default_value(here.string()), "root directory") // --generator.root
        ("generator.include-path,I",
         boost::program_options::value<std::vector<std::string>>()->default_value(std::vector<std::string>(1, root), root),
         "include paths")                                                                                      // --include-path
        ("generator.out", boost::program_options::value(&out_dir)->default_value(out_dir), "output directory") // --generator.out
        ("generator.cache",
         boost::program_options::value(&cache_dir)->default_value(cache_dir),
         "cache for temporary files") // --generator.cache
        ("generator.bs", boost::program_options::value(&bs_dir)->default_value(bs_dir), "local bootstrap sources") // --generator.bs
        ("generator.control",
         boost::program_options::value(&control_file)->default_value(control_file),
         "control file")                                                                                      // --generator.control
        ("generator.locale", boost::program_options::value(&def_locale)->default_value(def_locale), "locale") // --generator.locale
        ("generator.country",
         boost::program_options::value(&def_country)->default_value(def_country),
         "country") // --generator.country
        ("generator.language",
         boost::program_options::value(&def_lang)->default_value(def_lang),
         "language")                                                                                        // --generator.language
        ("generator.encoding", boost::program_options::value(&def_enc)->default_value(def_enc), "encoding") // --generator.encoding
        ;
    ...
    if (vm.count("version")) {
        std::cout << "doc2site generator v" << docc::version_string << std::endl;
        return EXIT_SUCCESS;
    }
    ...
    if (!ifs) {
        fmt::print(
            stdout, fg(fmt::color::dark_orange) | fmt::emphasis::bold, "***warn  : config file [{}] not found\n", config_file);
    } else {
        //
    ...
    fmt::print(stdout, fg(fmt::color::gray), "***info : verbose level = [{}]\n", verbose);

    //
    //	read specified include paths
    //
    auto const inc_paths = docscript::get_include_paths(vm["generator.include-path"].as<std::vector<std::string>>(), root);

    if (verbose > 1) {
        fmt::print(stdout, fg(fmt::color::gray), "***info : {} include paths\n", inc_paths.size());
    ...
    fmt::print(stderr, fg(fmt::color::crimson) | fmt::emphasis::bold, "***warn  : unknown command [{}]\n", cmd);
