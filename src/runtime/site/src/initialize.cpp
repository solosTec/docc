...
namespace docscript {
    initialize::initialize(std::vector<std::filesystem::path> inc, int verbose)
        : inc_(inc)
        , verbose_(verbose) {}
    ...
    int initialize::run(std::filesystem::path working_dir) {
        ...
        if (!std::filesystem::create_directories(working_dir / subdir_src / "css", ec)) {
            fmt::print(
                ...
                fg(fmt::color::dark_orange) | fmt::emphasis::bold,
                "***warn  : cannot create [{}]: {}\n",
                (working_dir / subdir_src / "css").string(),
                ec.message());
            return EXIT_FAILURE;
        }
        ...
        return cyng::make_tuple(
            ...
            cyng::make_param(
                ...
                cyng::make_vector({
                    ...
                    cyng::make_tuple(
                        cyng::make_param("title", "Home"), cyng::make_param("ref", "home"), cyng::make_param("enabled", true))})));
    }
    ...
    void initialize::download_bootstrap(std::filesystem::path, std::string version) const {
        ...
        //    req.target("/twbs/bootstrap/releases/download/v5.1.3/bootstrap-5.1.3-dist.zip");
        //    req.version(10);
        //    boost::beast::http::write(s, req);
        //}

        //{
        //    boost::beast::http::request<boost::beast::http::string_body> response;
        //    boost::beast::flat_buffer buf;
        //    boost::beast::http::read(s, buf, response);
        //}
    }
    ...
    void initialize::generate_logo(std::filesystem::path p) {
        ...
        if (img.is_open()) {
            ...
            fmt::print(stdout, fg(fmt::color::dark_orange) | fmt::emphasis::bold, "***warn  : cannot create: {}\n", p.string());
        }
    }
