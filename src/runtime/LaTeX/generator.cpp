...
namespace docruntime {
    ...
    void generator::set(cyng::param_map_t pm) { vars_.insert(pm.begin(), pm.end()); }
    ...
    void generator::embed_base64(
        ...
        // auto const title = compute_title_figure(tag, caption);
        // auto const figure = dom::figure(dom::id_(id), dom::img(dom::alt_(alt), dom::title_(caption),
        // dom::class_("docscript-img"), dom::style_("max-width: " + max_width), dom::src_("data:image/" + ext + ";base64," +
        // cyng::crypto::base64_encode(buffer.data(), buffer.size()))), dom::figcaption(title)); figure.serialize(os_);
    }

    void generator::table_of_content(cyng::param_map_t) { os_ << "\\tableofcontents" << std::endl; }
